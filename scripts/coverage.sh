...
# Use TEST_REPORTS_DIR if set, otherwise default to ./reports
...
PACKAGES=$(go list ./pkg/... ./internal/... ./cmd/... | grep -v -f ./scripts/coverage_ignore.txt)

echo "${BLUE}Running tests on packages:${NC}"
echo "$PACKAGES"

# Run the tests and generate coverage profile
go test -cover $PACKAGES -coverprofile="$TEST_REPORTS_DIR/coverage.out"
...
echo "${GREEN}HTML coverage report generated at: ${NC}$TEST_REPORTS_DIR/coverage.html"
