...
│   │   ├── http/in/            # Inbound HTTP handlers + routes
...
| **Repositories** (`internal/adapters/postgres`, `internal/adapters/cache`) | Data persistence, external system integration | Database drivers |
...
- All endpoints require authentication (except `/health`, `/ready`, `/version`, `/swagger/*`)
...
- `gocyclo` - Cyclomatic complexity (max: 16)
...
| Dependency | Purpose |
|------------|---------|
| `github.com/LerianStudio/lib-commons` | Common utilities, logging, tracing |
...
8. **Cyclomatic complexity > 16** - Refactor complex functions
