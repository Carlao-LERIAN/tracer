...
func TestIntegration_CircuitBreaker_DBFailure(t *testing.T) {
	...
	ruleCache.SetRules(cachedRules)
	ruleCache.MarkReady()
	...
	cachedResult := ruleCache.GetActiveRules(nil)
	assert.Len(t, cachedResult, 5, "cache should still serve original 5 rules despite DB failure")
	assert.True(t, ruleCache.IsReady(), "cache should still report ready (DEGRADED, not NOT_READY)")
...
func TestIntegration_CircuitBreaker_HealthRecovery(t *testing.T) {
	...
	assert.True(t, ruleCache.IsReady())
	...
	assert.True(t, ruleCache.IsReady(), "cache should still be ready when stale (DEGRADED)")
	assert.Equal(t, 60*time.Second, ruleCache.Staleness(), "cache should be stale")

	// 6. Simulate recovery: sync touches cache
	ruleCache.ApplyChanges(nil, nil) // touch lastSyncTime at advanced clock

	// 7. Verify recovery: staleness resets
	assert.Equal(t, time.Duration(0), ruleCache.Staleness(),
		"staleness should reset after successful sync (recovery)")
	assert.True(t, ruleCache.IsReady(), "cache should report ready after recovery")
}
