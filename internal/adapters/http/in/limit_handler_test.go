...
func TestLimitHandler_ListLimits(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "success - lists limits with defaults",
			queryParams: "",
			mockSetup: func(ctrl *gomock.Controller) *MockLimitService {
				mockService := NewMockLimitService(ctrl)
				mockService.EXPECT().
		...
		{
			name:        "success - lists limits with status filter ACTIVE",
		...
		{
			name:        "success - lists limits with status filter DRAFT",
...
func TestToListLimitsFilter(t *testing.T) {
	...
	assert.Equal(t, "createdAt", result.SortBy)
...
func TestListLimitsInput_SetDefaults(t *testing.T) {
	...
	assert.Equal(t, "createdAt", input.SortBy)
