...
func TestValidateCursorConsistency(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr:     true,
			wantCode:    "TRC-0045",
			wantMessage: "sortBy and sortOrder cannot be used with cursor; cursor already contains sort configuration",
		},
		{
...
func TestValidateSortOrder(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr:     true,
			wantCode:    "TRC-0042",
			wantMessage: "sortOrder must be ASC or DESC",
		},
		{
			name:        "invalid: number",
			sortOrder:   "123",
...
func TestValidateSortBy(t *testing.T) {
	allowedFields := []string{"createdAt", "updatedAt", "name", "status"}
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr:     true,
			wantCode:    "TRC-0043",
			wantMessage: "sortBy must be one of [createdAt updatedAt name status]",
		},
		{
