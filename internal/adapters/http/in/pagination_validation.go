...
// ValidateCursorConsistency checks if cursor is used with sortBy/sortOrder.
...
// already encoded in the cursor (which contains sortBy and sortOrder internally).
...
func ValidateSortBy(sortBy string, allowedFields []string) error {
	...
	return &ValidationError{
		Code:    "TRC-0043",
		Message: fmt.Sprintf("sortBy must be one of %v", allowedFields),
	}
}
