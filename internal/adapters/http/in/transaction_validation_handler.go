...
//	@Param			sortBy			query		string							false	"Field to sort by (default: createdAt)"		Enums(createdAt, processingTimeMs)
//	@Param			sortOrder		query		string							false	"Sort direction (default: DESC)"				Enums(ASC, DESC)
//	@Param			startDate		query		string							false	"Filter from this date (RFC3339)"				Format(date-time)
//	@Param			endDate			query		string							false	"Filter to this date (RFC3339)"					Format(date-time)
...
//	@Param			exceededLimitId	query		string							false	"Filter by exceeded limit ID (UUID)"			Format(uuid)
...
//	@Param			transactionType	query		string							false	"Filter by transaction type"					Enums(CARD, WIRE, PIX, CRYPTO)
...
type ListTransactionValidationsInput struct {
	...
	MatchedRuleID   string `query:"matchedRuleId"`
	ExceededLimitID string `query:"exceededLimitId"`
...
func (i *ListTransactionValidationsInput) Validate() error {
	...
	allowedSortFields := []string{"createdAt", "processingTimeMs"}
...
func (i *ListTransactionValidationsInput) validateDates() error {
	startDate, err := validation.ParseRFC3339Timestamp(i.StartDate, "startDate")
	if err != nil {
	...
	endDate, err := validation.ParseRFC3339Timestamp(i.EndDate, "endDate")
	if err != nil {
		return &ValidationError{
			Code:    "TRC-0020",
			Message: err.Error(),
...
func (i *ListTransactionValidationsInput) validateUUIDFilters() error {
	if err := validateUUID(i.AccountID, "accountId"); err != nil {
		return err
	}

	if err := validateUUID(i.MatchedRuleID, "matchedRuleId"); err != nil {
		return err
	}

	if err := validateUUID(i.ExceededLimitID, "exceededLimitId"); err != nil {
		return err
	}

	if err := validateUUID(i.SegmentID, "segmentId"); err != nil {
		return err
	}

	return validateUUID(i.PortfolioID, "portfolioId")
}
...
func (i *ListTransactionValidationsInput) validateTransactionType() error {
	if i.TransactionType != "" && !model.TransactionType(i.TransactionType).IsValid() {
		return errors.New("transactionType must be one of [CARD, WIRE, PIX, CRYPTO]")
	}
