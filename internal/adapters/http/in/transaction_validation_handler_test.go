	...
	tests := []struct {
	...
	}{
		{
			...
			expectedBody: func(t *testing.T, body []byte) {
				...
				assert.Equal(t, int64(42), response.ProcessingTimeMs)
...
func TestTransactionValidationHandler_ListTransactionValidations(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			queryParams: "?sortBy=invalid_field",
			mockSetup: func(ctrl *gomock.Controller) *mocks.MockTransactionValidationService {
				return mocks.NewMockTransactionValidationService(ctrl)
			},
			expectedStatus: http.StatusBadRequest,
			expectedBody: func(t *testing.T, body []byte) {
				assertStringErrorContains(t, body, "sortBy must be one of")
			},
		},
		{
			...
			expectedStatus: http.StatusBadRequest,
			expectedBody: func(t *testing.T, body []byte) {
				assertStringErrorContains(t, body, "sortOrder must be ASC or DESC")
			},
		},
		{
			...
			expectedBody: func(t *testing.T, body []byte) {
				// BadRequest returns simple string, not structured response
				assertStringErrorContains(t, body, "startDate must be in RFC3339 format")
			},
		},
		{
			name:        "error - invalid accountId UUID",
			queryParams: "?accountId=invalid-uuid",
			mockSetup: func(ctrl *gomock.Controller) *mocks.MockTransactionValidationService {
				return mocks.NewMockTransactionValidationService(ctrl)
			},
			expectedStatus: http.StatusBadRequest,
			expectedBody: func(t *testing.T, body []byte) {
				// BadRequest returns simple string, not structured response
				assertStringErrorContains(t, body, "accountId must be a valid UUID")
			},
		},
		{
		...
		},
		{
			...
			expectedStatus: http.StatusBadRequest,
			expectedBody: func(t *testing.T, body []byte) {
				assertStringErrorContains(t, body, "transactionType must be one of")
			},
		},
		{
			name:        "error - invalid segmentId UUID",
			queryParams: "?segmentId=invalid-uuid",
			mockSetup: func(ctrl *gomock.Controller) *mocks.MockTransactionValidationService {
				return mocks.NewMockTransactionValidationService(ctrl)
			},
			expectedStatus: http.StatusBadRequest,
			expectedBody: func(t *testing.T, body []byte) {
				assertStringErrorContains(t, body, "segmentId must be a valid UUID")
			},
		},
		{
			name:        "error - invalid portfolioId UUID",
			queryParams: "?portfolioId=invalid-uuid",
			mockSetup: func(ctrl *gomock.Controller) *mocks.MockTransactionValidationService {
				return mocks.NewMockTransactionValidationService(ctrl)
			},
			expectedStatus: http.StatusBadRequest,
			expectedBody: func(t *testing.T, body []byte) {
				assertStringErrorContains(t, body, "portfolioId must be a valid UUID")
			},
		},
		{
			name:        "success - with sortBy=processingTimeMs",
			queryParams: "?sortBy=processingTimeMs",
			mockSetup: func(ctrl *gomock.Controller) *mocks.MockTransactionValidationService {
				mockService := mocks.NewMockTransactionValidationService(ctrl)
				mockService.EXPECT().
					ListTransactionValidations(gomock.Any(), gomock.Any()).
					DoAndReturn(func(ctx interface{}, filters *model.TransactionValidationFilters) (*query.ListTransactionValidationsResult, error) {
						assert.Equal(t, "processingTimeMs", filters.SortBy)
						return &query.ListTransactionValidationsResult{
							TransactionValidations: audits,
							NextCursor:             "",
							HasMore:                false,
...
func TestToTransactionValidationFilters(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedFilter: func(t *testing.T, f *model.TransactionValidationFilters) {
				assert.Equal(t, "createdAt", f.SortBy)
...
func TestListTransactionValidationsInput_Validate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "sortBy must be one of [createdAt processingTimeMs]",
		},
		{
			name: "valid - sortBy processingTimeMs",
			input: ListTransactionValidationsInput{
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "sortOrder must be ASC or DESC",
		},
		{
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "startDate must be in RFC3339 format",
		},
		{
			...
			wantErr: true,
			errMsg:  "endDate must be in RFC3339 format",
		},
		{
			name: "error - invalid accountId UUID",
			input: ListTransactionValidationsInput{
			...
			wantErr: true,
			errMsg:  "accountId must be a valid UUID",
		},
		{
			...
			wantErr: true,
			errMsg:  "matchedRuleId must be a valid UUID",
		},
		{
			...
			wantErr: true,
			errMsg:  "exceededLimitId must be a valid UUID",
		},
		{
			name: "error - invalid segmentId UUID",
			input: ListTransactionValidationsInput{
			...
			wantErr: true,
			errMsg:  "segmentId must be a valid UUID",
		},
		{
			name: "error - invalid portfolioId UUID",
			input: ListTransactionValidationsInput{
			...
			wantErr: true,
			errMsg:  "portfolioId must be a valid UUID",
		},
		{
			...
			wantErr: true,
			errMsg:  "transactionType must be one of [CARD, WIRE, PIX, CRYPTO]",
		},
		{
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "startDate must be before or equal to endDate",
		},
		{
