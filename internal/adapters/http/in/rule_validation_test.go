...
// TestToListFilter_SortBy_PassesCamelCaseToFilter verifies that SortBy values are passed
// as camelCase to the filter (repository handles conversion to snake_case).
func TestToListFilter_SortBy_PassesCamelCaseToFilter(t *testing.T) {
	tests := []struct {
		name           string
		inputSortBy    string
		...
		{
			name:           "createdAt passed as-is",
			inputSortBy:    "createdAt",
			expectedSortBy: "createdAt",
		},
		{
			name:           "updatedAt passed as-is",
			inputSortBy:    "updatedAt",
			expectedSortBy: "updatedAt",
		},
		{
