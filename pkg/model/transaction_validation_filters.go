...
// Uses camelCase convention per API design standards.
var validTransactionValidationSortFields = map[string]bool{
	"createdAt":        true, // Default sort field
	"processingTimeMs": true, // Sort by processing time
}
...
type TransactionValidationFilters struct {
	...
	// SortBy is the field to sort by. Defaults to "createdAt".
...
func (f *TransactionValidationFilters) Validate() error {
	...
	if !f.StartDate.IsZero() && !f.EndDate.IsZero() {
		if f.EndDate.Before(f.StartDate) {
			return fmt.Errorf("%w: endDate must be after startDate", constant.ErrInvalidTransactionValidationFilters)
		}
	}
	...
	if f.SortBy != "" && !IsValidTransactionValidationSortField(f.SortBy) {
		return fmt.Errorf("%w: invalid sortBy field", constant.ErrInvalidTransactionValidationFilters)
	}
	...
	if f.SortOrder != "" {
		...
		if normalizedOrder != "ASC" && normalizedOrder != "DESC" {
			return fmt.Errorf("%w: sortOrder must be ASC or DESC", constant.ErrInvalidTransactionValidationFilters)
		}
	}

	// Validate TransactionType if provided
	if f.TransactionType != nil && !f.TransactionType.IsValid() {
		return fmt.Errorf("%w: invalid transactionType", constant.ErrInvalidTransactionValidationFilters)
	}
