...
func (f *AuditEventFilters) Validate() error {
	if !f.StartDate.IsZero() && !f.EndDate.IsZero() {
		if f.EndDate.Before(f.StartDate) {
			return fmt.Errorf("%w: endDate must be after startDate", constant.ErrInvalidAuditEventFilters)
		}
	}
	...
	if f.SortBy != "" && !IsValidAuditEventSortField(f.SortBy) {
		return fmt.Errorf("%w: invalid sortBy field", constant.ErrInvalidAuditEventFilters)
	}

	if f.SortOrder != "" {
		normalizedOrder := strings.ToUpper(f.SortOrder)
		if normalizedOrder != "ASC" && normalizedOrder != "DESC" {
			return fmt.Errorf("%w: sortOrder must be ASC or DESC", constant.ErrInvalidAuditEventFilters)
		}
	}
