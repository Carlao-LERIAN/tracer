...
func TestAuditEventFilters_Validate(t *testing.T) {
	tests := []struct {
		...
		{
			name: "success - valid sortBy createdAt",
			filters: &AuditEventFilters{
				Limit:  100,
				SortBy: "createdAt",
			},
		...
		{
			name: "success - valid sortBy eventType",
			filters: &AuditEventFilters{
				Limit:  100,
				SortBy: "eventType",
			},
		...
		},
		{
			...
			expectErr: true,
			errorMsg:  "endDate must be after startDate",
		},
		{
		...
		},
		{
			...
			expectErr: true,
			errorMsg:  "sortOrder must be ASC or DESC",
		},
		{
...
func TestAuditEventFilters_SetDefaults(t *testing.T) {
	...
	t.Run("sets default sortBy to createdAt", func(t *testing.T) {
		filters := &AuditEventFilters{}
	...
	t.Run("preserves non-empty sortBy", func(t *testing.T) {
		...
		assert.Equal(t, "eventType", filters.SortBy)
	})
...
func TestIsValidAuditEventSortField(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "createdAt is valid (camelCase)",
			field:    "createdAt",
			expected: true,
		},
		{
			name:     "eventType is valid (camelCase)",
			field:    "eventType",
			expected: true,
		},
		{
		...
		{
			name:     "snake_case is not accepted",
			field:    "created_at",
			expected: false,
		},
		{
			name:     "snake_case eventType is not accepted",
			field:    "event_type",
			expected: false,
		},
	}
