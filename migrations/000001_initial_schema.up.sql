...
-- Enum types
-- Note: Removed DO $$ block due to golang-migrate bug #590 with dollar-quoting
-- Since we have a single migration and no production yet, direct creation is safe
...
-- Note: Requires prevent_truncate() function from migrations/functions/
CREATE TRIGGER prevent_transaction_validation_truncate_trigger
...
--
-- The prevent_truncate() function is created by migrations/functions/000003_prevent_truncate.up.sql
-- and triggers are created above (lines 139, 295).
--
-- FOR PRODUCTION:
...
-- Note: Run migrations/functions/*.up.sql before this migration
-- Or execute via \i migrations/functions/001_calculate_audit_event_hash.up.sql
-- Or execute via \i migrations/functions/002_verify_audit_hash_chain.up.sql
...
-- Note: Requires calculate_audit_event_hash() function from migrations/functions/
