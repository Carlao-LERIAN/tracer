...
func TestValidation_CompletePayload(t *testing.T) {
	...
	assert.GreaterOrEqual(t, result.ProcessingTimeMs, int64(0), "processingTimeMs should be >= 0")
...
func TestValidation_ReturnsDenyWhenLimitExceeded(t *testing.T) {
	...
	// Create and activate DAILY limit with amount 100000 for accountId
	limitID := createTestLimitWithAccountScope(t, accountID, 100000)
	activateTestLimit(t, limitID)
	t.Cleanup(func() {
		cleanupTestLimit(t, limitID)
	})

	// Consume 90000 of the limit with a first validation
	...
	// Now try to exceed the limit with amount 20000 (90000 + 20000 > 100000)
	...
	for _, detail := range result.LimitUsageDetails {
		if detail.LimitID == limitID {
			...
			assert.Contains(t, detail.Scope, accountID, "scope should contain accountId")

			// Validate period
			assert.Equal(t, "DAILY", detail.Period, "period should be DAILY")

			// Validate amounts
			assert.Equal(t, int64(100000), detail.LimitAmount, "limitAmount should be 100000")
			assert.GreaterOrEqual(t, detail.CurrentUsage, int64(90000), "currentUsage should be at least 90000")
			assert.Equal(t, int64(20000), detail.AttemptedAmount, "attemptedAmount should be 20000")
...
func TestValidation_DecisionPrecedence(t *testing.T) {
	...
	t.Run("Limit exceeded takes precedence over REVIEW rule", func(t *testing.T) {
		...
		limitID := createTestLimitWithAccountScope(t, accountID, 5000)
		activateTestLimit(t, limitID)
...
func TestValidation_1_1_8_MultipleMatchingRules(t *testing.T) {
	...
	// Rule 2: matches amount > 5000
	rule2Name := "allow-amount-rule2-" + testutil.MustDeterministicUUID(303).String()[:8]
	rule2ID := testutil.CreateTestRuleWithExpression(t, rule2Name, "amount > 5000", "ALLOW")
...
func TestValidation_InvalidAmount(t *testing.T) {
	...
	for i, tc := range invalidTests {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, http.StatusBadRequest, resp.StatusCode, "Amount %d should return 400", tc.amount)
...
func TestValidation_1_1_29_RejectsDecimalAmount(t *testing.T) {
	...
	assert.Equal(t, http.StatusBadRequest, resp.StatusCode,
		"Decimal amount should return 400 Bad Request")

	// Verify error response structure
	errorResp := testutil.ParseErrorResponse(t, body)
	assert.Equal(t, "TRC-0003", errorResp.Code, "Error code should be TRC-0003 for decimal amount (body parsing error)")
	assert.Equal(t, "Bad Request", errorResp.Title, "Error title should be Bad Request")
	assert.Equal(t, "invalid request body", errorResp.Message, "Error message should indicate invalid request body")
}
...
func TestValidation_1_1_55_DeactivatedRuleNotEvaluated(t *testing.T) {
	...
	// Step 1 (Rule ACTIVE): Validate with amount=250000, expect DENY
...
func TestValidation_1_2_1_RetrievesValidationByID(t *testing.T) {
	...
	assert.Equal(t, int64(50000), result.Amount, "amount should be preserved")
...
func TestValidation_1_2_5_CompleteSnapshotPreserved(t *testing.T) {
	...
	assert.Equal(t, int64(75000), result.Amount, "amount should be preserved")
	assert.Equal(t, "BRL", result.Currency, "currency should be preserved")
...
func TestValidation_1_2_7_CompleteDataWithSegmentPortfolio(t *testing.T) {
	...
	assert.Equal(t, "CARD", result.TransactionType, "transactionType should be preserved")
	assert.Equal(t, int64(85000), result.Amount, "amount should be preserved")
...
func TestValidation_1_2_8_CompleteLimitUsagePreserved(t *testing.T) {
	...
	assert.Equal(t, maxAmount, foundLimit.LimitAmount, "limitAmount should be the configured max amount")
	assert.Equal(t, consumeAmount, foundLimit.CurrentUsage, "currentUsage should reflect the consumed amount")
	...
	assert.Contains(t, foundLimit.Scope, accountID, "scope should contain the accountId")
...
func TestValidation_1_3_2_FiltersByDateRange(t *testing.T) {
	...
	listResp, listBody := testutil.ListValidations(t, fmt.Sprintf("startDate=%s&endDate=%s", startDate, endDate))
	defer listResp.Body.Close()
...
func TestValidation_1_3_4_FiltersByAccountID(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, listResp.StatusCode,
		"Expected 200 OK from GET /v1/validations?accountId=..., got: %s", string(listBody))
	...
	for _, item := range result.TransactionValidations {
		assert.Equal(t, accountID, item.AccountID,
			"All items should have accountId == %s when filtering by accountId", accountID)
	}

	// Verify we got at least the validation we just created
	assert.GreaterOrEqual(t, len(result.TransactionValidations), 1,
		"Should have at least 1 validation for this accountId")
}
...
func TestValidation_1_3_5_FiltersBySegmentID(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, listResp.StatusCode,
		"Expected 200 OK from GET /v1/validations?segmentId=..., got: %s", string(listBody))
...
func TestValidation_1_3_6_FiltersByPortfolioID(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, listResp.StatusCode,
		"Expected 200 OK from GET /v1/validations?portfolioId=..., got: %s", string(listBody))
...
func TestValidation_1_3_7_FiltersByTransactionType(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, listResp.StatusCode,
		"Expected 200 OK from GET /v1/validations?transactionType=CARD, got: %s", string(listBody))
	...
	for _, item := range result.TransactionValidations {
		assert.Equal(t, "CARD", item.TransactionType,
			"All items should have transactionType == CARD when filtering by transactionType=CARD")
	}

	// Verify we got at least the validation we just created
	assert.GreaterOrEqual(t, len(result.TransactionValidations), 1,
		"Should have at least 1 validation with transactionType=CARD")
}
...
func TestValidation_1_3_8_FiltersByRuleID(t *testing.T) {
	...
	listResp, listBody := testutil.ListValidations(t, fmt.Sprintf("matchedRuleId=%s", ruleID))
	defer listResp.Body.Close()

	require.Equal(t, http.StatusOK, listResp.StatusCode,
		"Expected 200 OK from GET /v1/validations?matchedRuleId=..., got: %s", string(listBody))
...
func TestValidation_1_3_9_FiltersByExceededLimitId(t *testing.T) {
	...
	maxAmount := int64(10000) // Very low limit
	limitID := createTestLimitWithAccountScope(t, accountID, maxAmount)
	activateTestLimit(t, limitID)
	t.Cleanup(func() {
		cleanupTestLimit(t, limitID)
	...
	listResp, listBody := testutil.ListValidations(t, fmt.Sprintf("exceededLimitId=%s", limitID))
	defer listResp.Body.Close()

	require.Equal(t, http.StatusOK, listResp.StatusCode,
		"Expected 200 OK from GET /v1/validations?exceededLimitId=..., got: %s", string(listBody))
...
func TestValidation_1_3_10_PaginationWorks(t *testing.T) {
	...
	listResp1, listBody1 := testutil.ListValidations(t, fmt.Sprintf("accountId=%s&limit=10", baseAccountID))
	...
	if result1.NextCursor != "" {
		listResp2, listBody2 := testutil.ListValidations(t,
			fmt.Sprintf("accountId=%s&limit=10&cursor=%s", baseAccountID, result1.NextCursor))
...
func TestValidation_1_3_11_SortingWorks(t *testing.T) {
	...
	// Query with sortBy=processingTimeMs&sortOrder=DESC
	listResp, listBody := testutil.ListValidations(t,
		fmt.Sprintf("accountId=%s&sortBy=processingTimeMs&sortOrder=DESC", accountID))
	defer listResp.Body.Close()
...
func TestValidation_1_3_12_CombinedFilters(t *testing.T) {
	...
	req := &testutil.ValidationRequest{
		...
		TransactionType:      "CARD",
		Amount:               100000, // Will trigger DENY rule
		Currency:             "BRL",
		TransactionTimestamp: now.Format(time.RFC3339),
		Account: &testutil.AccountContext{
			ID: accountID,
	...
	listResp, listBody := testutil.ListValidations(t,
		fmt.Sprintf("decision=DENY&transactionType=CARD&startDate=%s&accountId=%s", startDate, accountID))
	defer listResp.Body.Close()
	...
	for _, item := range result.TransactionValidations {
		...
		assert.Equal(t, accountID, item.AccountID,
			"All items should have matching accountId")
...
func TestValidation_1_3_16_RejectsInvalidSegmentIdFilter(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			listResp, _ := testutil.ListValidations(t, fmt.Sprintf("segmentId=%s", tc.segmentID))
			defer listResp.Body.Close()
...
func TestValidation_1_3_17_RejectsInvalidPortfolioIdFilter(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			listResp, _ := testutil.ListValidations(t, fmt.Sprintf("portfolioId=%s", tc.portfolioID))
			defer listResp.Body.Close()
...
func TestValidation_1_3_18_RejectsInvalidAccountIdFilter(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			listResp, _ := testutil.ListValidations(t, fmt.Sprintf("accountId=%s", tc.accountID))
			defer listResp.Body.Close()
...
func TestValidation_1_3_19_RejectsInvalidRuleIdFilter(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			listResp, _ := testutil.ListValidations(t, fmt.Sprintf("matchedRuleId=%s", tc.ruleID))
			defer listResp.Body.Close()
...
func TestValidation_1_3_20_RejectsInvalidTransactionTypeFilter(t *testing.T) {
	...
	listResp, _ := testutil.ListValidations(t, "transactionType=INVALID_TYPE")
	defer listResp.Body.Close()

	assert.Equal(t, http.StatusBadRequest, listResp.StatusCode,
		"Invalid transactionType filter should return 400 Bad Request")
}
...
func TestValidation_1_3_22_RejectsInvalidSortBy(t *testing.T) {
	...
	listResp, _ := testutil.ListValidations(t, "sortBy=invalid_field")
	defer listResp.Body.Close()

	assert.Equal(t, http.StatusBadRequest, listResp.StatusCode,
		"Invalid sortBy field should return 400 Bad Request")
}
...
func TestValidation_1_3_23_RejectsInvalidSortOrder(t *testing.T) {
	...
	listResp, _ := testutil.ListValidations(t, "sortOrder=INVALID")
	defer listResp.Body.Close()
...
func TestValidation_1_3_26_RejectsStartDateAfterEndDate(t *testing.T) {
	...
	listResp, _ := testutil.ListValidations(t, "startDate=2025-01-20T00:00:00Z&endDate=2025-01-10T00:00:00Z")
	defer listResp.Body.Close()
...
func TestValidation_1_3_13_SortingByProcessingTimeAsc(t *testing.T) {
	...
	// Query with sortBy=processingTimeMs&sortOrder=ASC
	listResp, listBody := testutil.ListValidations(t,
		fmt.Sprintf("accountId=%s&sortBy=processingTimeMs&sortOrder=ASC", accountID))
	defer listResp.Body.Close()
...
func TestValidation_1_3_27_CombinedFiltersWithNewParams(t *testing.T) {
	...
	queryParams := fmt.Sprintf(
		"decision=%s&transactionType=CARD&startDate=%s&endDate=%s&accountId=%s&limit=10",
	...
	for _, item := range result.TransactionValidations {
		...
		assert.Equal(t, "CARD", item.TransactionType,
			"All items should have transactionType == CARD")
		assert.Equal(t, accountID, item.AccountID,
			"All items should have accountId == %s", accountID)
...
func TestValidation_1_2_11_ProcessingTimeMsNonNegative(t *testing.T) {
	...
	assert.Less(t, result.ProcessingTimeMs, int64(3600000),
...
func TestValidation_1_3_14_CombinedFiltersAdvanced(t *testing.T) {
	...
	// Query with combined filters: accountId=X&decision=DENY&transactionType=CARD
	listResp, listBody := testutil.ListValidations(t,
		fmt.Sprintf("accountId=%s&decision=DENY&transactionType=CARD", accountID))
	defer listResp.Body.Close()
	...
	// Verify all items satisfy ALL filters
	for _, item := range result.TransactionValidations {
		assert.Equal(t, accountID, item.AccountID, "All items should have matching accountId")
...
func TestValidation_1_3_30_RejectsInvalidExceededLimitIdFilter(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			listResp, _ := testutil.ListValidations(t, fmt.Sprintf("exceededLimitId=%s", tc.limitID))
			defer listResp.Body.Close()
...
func TestValidation_1_3_31_EmptyResultSetValidStructure(t *testing.T) {
	...
	listResp, listBody := testutil.ListValidations(t, fmt.Sprintf("accountId=%s", nonExistentAccountID))
	defer listResp.Body.Close()
...
func TestValidation_1_3_33_SortingByCreatedAtAsc(t *testing.T) {
	...
	// Query with sortBy=createdAt&sortOrder=ASC
	listResp, listBody := testutil.ListValidations(t,
		fmt.Sprintf("accountId=%s&sortBy=createdAt&sortOrder=ASC", accountID))
	defer listResp.Body.Close()
...
func TestValidation_1_3_34_MultipleParametersSimultaneously(t *testing.T) {
	...
	queryParams := fmt.Sprintf(
		"startDate=%s&endDate=%s&decision=%s&accountId=%s",
...
func TestValidation_1_3_38_RejectsInvalidStartDateFormat(t *testing.T) {
	listResp, _ := testutil.ListValidations(t, "startDate=not-a-date")
	defer listResp.Body.Close()
...
func TestValidation_1_3_39_RejectsInvalidEndDateFormat(t *testing.T) {
	listResp, _ := testutil.ListValidations(t, "endDate=not-a-date")
	defer listResp.Body.Close()
...
func TestValidation_1_3_40_CursorPaginationConsistency(t *testing.T) {
	...
	for pageCount < maxPages {
		queryParams := fmt.Sprintf("accountId=%s&limit=10", accountID)
		if cursor != "" {
...
func TestValidation_1_3_41_FilterByMatchedRuleId(t *testing.T) {
	...
	// Filter by ruleId
	listResp, listBody := testutil.ListValidations(t,
		fmt.Sprintf("accountId=%s&matchedRuleId=%s", accountID, ruleID))
	defer listResp.Body.Close()
...
func TestValidation_1_3_42_AllSortBySortOrderCombinations(t *testing.T) {
	...
	testCases := []struct {
		...
		{"processingTimeMs ASC", "processingTimeMs", "ASC"},
		{"processingTimeMs DESC", "processingTimeMs", "DESC"},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			listResp, listBody := testutil.ListValidations(t,
				fmt.Sprintf("accountId=%s&sortBy=%s&sortOrder=%s", accountID, tc.sortBy, tc.sortOrder))
			defer listResp.Body.Close()

			require.Equal(t, http.StatusOK, listResp.StatusCode,
				"Expected 200 OK for sortBy=%s&sortOrder=%s, got: %s", tc.sortBy, tc.sortOrder, string(listBody))
			...
			if len(result.TransactionValidations) >= 2 {
				for i := 1; i < len(result.TransactionValidations); i++ {
					...
					} else if tc.sortBy == "processingTimeMs" {
						if tc.sortOrder == "ASC" {
...
func TestValidation_1_3_43_DateRangeBoundarySemantics(t *testing.T) {
	...
	t.Run("startDate is inclusive", func(t *testing.T) {
		...
		listResp, listBody := testutil.ListValidations(t,
			fmt.Sprintf("accountId=%s&startDate=%s", accountID, startDate))
		defer listResp.Body.Close()
	...
	t.Run("endDate is exclusive", func(t *testing.T) {
		...
		listResp, listBody := testutil.ListValidations(t,
			fmt.Sprintf("accountId=%s&endDate=%s", accountID, endDate))
		defer listResp.Body.Close()
...
func TestValidation_1_3_29_ValidationSummaryFields(t *testing.T) {
	...
	for i, item := range result.TransactionValidations {
		...
		assert.GreaterOrEqual(t, item.Amount, int64(0),
			"Item %d: amount should be >= 0, got: %d", i, item.Amount)
		...
		assert.GreaterOrEqual(t, item.ProcessingTimeMs, int64(0),
			"Item %d: processingTimeMs should be >= 0, got: %d", i, item.ProcessingTimeMs)
