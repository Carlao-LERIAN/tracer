...
func TestListLimits_4_2_FilterByAccountId(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/limits?accountId=%s", baseURL, accountID)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	limitIDs := extractLimitIDsFromList(t, result.Limits)
	assert.Contains(t, limitIDs, limitMatch, "Should contain limit with matching accountId scope")
	assert.NotContains(t, limitIDs, limitNoMatch, "Should NOT contain limit with different accountId scope")
}
...
func TestListLimits_4_3_FilterByTransactionType(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/limits?transactionType=%s", baseURL, txType)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	limitIDs := extractLimitIDsFromList(t, result.Limits)
	assert.Contains(t, limitIDs, limitCard, "Should contain limit with CARD transactionType")
	assert.NotContains(t, limitIDs, limitPix, "Should NOT contain limit with PIX transactionType")
}
...
func TestListLimits_4_4_FilterByMultipleScopeFields(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/limits?accountId=%s&transactionType=%s", baseURL, accountID, txType)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	assert.Contains(t, limitIDs, limitAccountOnly, "Should contain limit with wildcard transactionType")
	assert.Contains(t, limitIDs, limitTxTypeOnly, "Should contain limit with wildcard accountId")
}
...
func TestListLimits_4_5_ScopeFiltersWithExistingFilters(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/limits?accountId=%s&status=ACTIVE", baseURL, accountID)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
...
func TestListLimits_4_6_InvalidScopeFiltersReturnError(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			queryParams: "segmentId=invalid",
			expectCode:  "TRC-0006",
			expectMsg:   "segmentId",
		},
		{
			name:        "invalid portfolioId UUID",
			queryParams: "portfolioId=bad-uuid",
			expectCode:  "TRC-0006",
			expectMsg:   "portfolioId",
		},
		{
			name:        "invalid merchantId UUID",
			queryParams: "merchantId=xyz",
			expectCode:  "TRC-0006",
			expectMsg:   "merchantId",
		},
		{
			name:        "invalid transactionType enum",
			queryParams: "transactionType=INVALID_TYPE",
			expectCode:  "TRC-0006",
			expectMsg:   "transactionType",
		},
		{
			name:        "subType exceeds max length",
			queryParams: "subType=" + strings.Repeat("x", 51),
			expectCode:  "TRC-0006",
			expectMsg:   "subType",
		},
	}
...
func TestListLimits_4_8_FilterBySegmentId(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/limits?segmentId=%s", baseURL, segmentID)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	limitIDs := extractLimitIDsFromList(t, result.Limits)
	assert.Contains(t, limitIDs, limitMatch, "Should contain limit with matching segmentId")
	assert.NotContains(t, limitIDs, limitNoMatch, "Should NOT contain limit with different segmentId")
}
...
func TestListLimits_4_9_ScopeFilterWithPagination(t *testing.T) {
	...
	// First page: limit=2
	url := fmt.Sprintf("%s/v1/limits?segmentId=%s&limit=2", baseURL, segmentID)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	url2 := fmt.Sprintf("%s/v1/limits?segmentId=%s&limit=2&cursor=%s", baseURL, segmentID, result1.NextCursor)
