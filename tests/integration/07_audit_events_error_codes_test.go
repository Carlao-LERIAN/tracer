...
func TestListAuditEvents_InvalidSortColumn_ReturnsTRC0043(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			queryParams: "sortBy=invalidColumn",
			desc:        "Non-existent column name",
		},
		{
			name:        "random_string",
			queryParams: "sortBy=xyz123",
...
func TestListAuditEvents_InvalidFilters_ReturnsValidationError(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			queryParams:   "eventType=INVALID_EVENT_TYPE",
		...
		},
		{
			...
			queryParams:   "resourceType=invalid_type",
			expectedCode:  "TRC-0001",
			expectedTitle: "Validation Error",
		...
		},
		{
			...
			queryParams:   "startDate=not-a-date",
		...
		},
		{
			...
			queryParams:   "endDate=2024-13-45",
			expectedCode:  "TRC-0020",
			expectedTitle: "Validation Error",
...
func TestListAuditEvents_CursorWithSortParams_ReturnsTRC0045(t *testing.T) {
	...
	t.Run("cursor_with_sortBy", func(t *testing.T) {
		req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?cursor="+listResp.NextCursor+"&sortBy=createdAt", nil)
		require.NoError(t, err)
		req.Header.Set("X-API-Key", apiKey)
	...
	t.Run("cursor_with_sortOrder", func(t *testing.T) {
		req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?cursor="+listResp.NextCursor+"&sortOrder=DESC", nil)
		require.NoError(t, err)
		req.Header.Set("X-API-Key", apiKey)
	...
	t.Run("cursor_with_both_sort_params", func(t *testing.T) {
		req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?cursor="+listResp.NextCursor+"&sortBy=createdAt&sortOrder=ASC", nil)
		require.NoError(t, err)
		req.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_ValidSortColumns_Succeeds(t *testing.T) {
	...
	validSortColumns := []string{"createdAt", "eventType"}

	for _, column := range validSortColumns {
		t.Run("sortBy_"+column, func(t *testing.T) {
			req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?sortBy="+column+"&limit=1", nil)
			require.NoError(t, err)
			req.Header.Set("X-API-Key", apiKey)
			...
			assert.Equal(t, http.StatusOK, resp.StatusCode, "sortBy=%s should succeed - Response: %s", column, string(respBody))
		})
	}
}
...
func TestAuditEvents_ValidSortOrders_Succeeds(t *testing.T) {
	...
	for _, order := range validSortOrders {
		t.Run("sortOrder_"+order, func(t *testing.T) {
			req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?sortOrder="+order+"&limit=1", nil)
			require.NoError(t, err)
			req.Header.Set("X-API-Key", apiKey)
			...
			assert.Equal(t, http.StatusOK, resp.StatusCode, "sortOrder=%s should succeed - Response: %s", order, string(respBody))
		})
	}
}
