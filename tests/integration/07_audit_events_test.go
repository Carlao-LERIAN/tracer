...
func TestAuditEvents_11_1_1_RetrievesAuditEventByID(t *testing.T) {
	...
	ruleName := "audit-test-rule-" + uuid.New().String()[:8]
	...
	listReq, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?resourceType=rule&action=CREATE&resourceId="+rule.ID, nil)
	require.NoError(t, err)
...
func TestAuditEvents_11_1_4_Returns401WithoutAPIKey(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events/"+uuid.New().String(), nil)
	require.NoError(t, err)
	// No API Key header
...
func TestAuditEvents_11_2_2_FiltersByEventType(t *testing.T) {
	...
	ruleName := "filter-test-" + uuid.New().String()[:8]
	ruleReq := testutil.RuleRequest{
		Name:       ruleName,
		Expression: "true",
		Action:     "ALLOW",
	...
	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?eventType=RULE_CREATED", nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_2_5_FiltersByResourceType(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?resourceType=rule", nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_2_6_FiltersByDateRange(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/audit-events?startDate=%s&endDate=%s", baseURL, startDate, endDate)
...
// TestAuditEvents_11_2_7_FiltersByAccountId tests filtering by accountId (JSONB query).
func TestAuditEvents_11_2_7_FiltersByAccountId(t *testing.T) {
	...
	// Filter by accountId
	url := fmt.Sprintf("%s/v1/audit-events?accountId=%s&eventType=TRANSACTION_VALIDATED", baseURL, accountID)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	require.NotEmpty(t, result.AuditEvents, "Should find audit event for accountId")
	...
	for _, event := range result.AuditEvents {
		...
		assert.Equal(t, accountID, account["id"], "All events should match accountId filter")
	}
}
...
func TestAuditEvents_11_2_8_FiltersByTransactionType(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/audit-events?transactionType=PIX&eventType=TRANSACTION_VALIDATED", baseURL)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	require.NotEmpty(t, result.AuditEvents, "Should find audit events for transactionType")
...
func TestAuditEvents_11_2_9_FiltersByMatchedRuleId(t *testing.T) {
	...
	ruleName := "matched-rule-" + uuid.New().String()[:8]
	...
	url := fmt.Sprintf("%s/v1/audit-events?matchedRuleId=%s&eventType=TRANSACTION_VALIDATED", baseURL, ruleID)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_2_10_A_ReturnsAllResultsWithoutPagination(t *testing.T) {
	...
	var ruleIDs []string
	for i := 0; i < 2; i++ {
		ruleName := fmt.Sprintf("no-pagination-test-%d-%s", i, uuid.New().String()[:8])
		ruleReq := testutil.RuleRequest{
			Name:       ruleName,
			Expression: "true",
			Action:     "ALLOW",
	...
	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?limit=100&resourceType=rule&action=CREATE", nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_2_10_B_IteratesThroughMultiplePages(t *testing.T) {
	...
	var ruleIDs []string
	for i := 0; i < 7; i++ {
		ruleName := fmt.Sprintf("pagination-test-%d-%s", i, uuid.New().String()[:8])
		ruleReq := testutil.RuleRequest{
			Name:       ruleName,
			Expression: "true",
			Action:     "ALLOW",
	...
	req1, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?limit=2&resourceType=rule&action=CREATE", nil)
	require.NoError(t, err)
...
func TestAuditEvents_11_2_11_SupportsCustomSorting(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?sortBy=eventType&sortOrder=ASC&limit=10", nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	if len(result.AuditEvents) >= 2 {
		for i := 0; i < len(result.AuditEvents)-1; i++ {
			...
			assert.LessOrEqual(t, curr, next, "Events should be sorted by eventType ASC")
		}
	}
}
...
func TestAuditEvents_11_2_12_Returns400ForInvalidFilters(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?eventType=INVALID_TYPE", nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_2_13_ReturnsEmptyArrayWhenNoMatches(t *testing.T) {
	...
	nonExistentID := "00000000-0000-0000-0000-000000000000"
	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?accountId="+nonExistentID, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_3_1_VerifiesValidHashChain(t *testing.T) {
	...
	ruleName := "verify-hash-" + uuid.New().String()[:8]
	ruleReq := testutil.RuleRequest{
		Name:       ruleName,
		Expression: "true",
		Action:     "ALLOW",
...
func TestAuditEvents_11_4_1_GeneratesAuditForRuleCreation(t *testing.T) {
	...
	ruleName := "audit-gen-test-" + uuid.New().String()[:8]
	ruleReq := testutil.RuleRequest{
		Name:       ruleName,
		Expression: "amount > 1000",
		Action:     "DENY",
	...
	// Query audit events
	auditReq, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?resourceType=rule&action=CREATE&resourceId="+rule.ID, nil)
	require.NoError(t, err)
...
func TestAuditEvents_11_4_2_GeneratesAuditForRuleActivation(t *testing.T) {
	...
	ruleName := "activate-audit-" + uuid.New().String()[:8]
	ruleReq := testutil.RuleRequest{
		Name:       ruleName,
		Expression: "true",
		Action:     "ALLOW",
	...
	auditReq, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?resourceId="+rule.ID+"&eventType=RULE_ACTIVATED", nil)
	require.NoError(t, err)
	auditReq.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_4_3_GeneratesAuditForRuleUpdate(t *testing.T) {
	...
	// Create rule
	ruleName := "update-audit-" + uuid.New().String()[:8]
	ruleReq := testutil.RuleRequest{
		Name:       ruleName,
		Expression: "amount > 1000",
		Action:     "DENY",
	...
	// Query audit events
	auditReq, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?resourceId="+rule.ID+"&eventType=RULE_UPDATED", nil)
	require.NoError(t, err)
	auditReq.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_4_4_GeneratesAuditForRuleDelete(t *testing.T) {
	...
	ruleName := "delete-audit-" + uuid.New().String()[:8]
	ruleReq := testutil.RuleRequest{
		Name:       ruleName,
		Expression: "true",
		Action:     "ALLOW",
	...
	// Query audit events
	auditReq, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?resourceId="+rule.ID+"&eventType=RULE_DELETED", nil)
	require.NoError(t, err)
	auditReq.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_4_5_GeneratesAuditForTransactionValidation(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/audit-events?eventType=TRANSACTION_VALIDATED&accountId=%s", baseURL, accountID)
...
func TestAuditEvents_11_4_6_CapturesClientIPInAuditEvents(t *testing.T) {
	...
	ruleName := "ip-test-" + uuid.New().String()[:8]
	ruleReq := testutil.RuleRequest{
		Name:       ruleName,
		Expression: "true",
		Action:     "ALLOW",
	...
	// Query audit event
	auditReq, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?resourceId="+rule.ID+"&action=CREATE", nil)
	require.NoError(t, err)
	auditReq.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_4_7_GeneratesAuditForLimitCreation(t *testing.T) {
	...
	listReq, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?resourceType=limit&action=CREATE&resourceId="+limitID, nil)
	require.NoError(t, err)
	listReq.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_4_8_GeneratesAuditForLimitActivation(t *testing.T) {
	...
	auditReq, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?resourceId="+limitID+"&eventType=LIMIT_ACTIVATED", nil)
	require.NoError(t, err)
	auditReq.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_4_9_GeneratesAuditForLimitUpdate(t *testing.T) {
	...
	// Query audit events
	auditReq, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?resourceId="+limitID+"&eventType=LIMIT_UPDATED", nil)
	require.NoError(t, err)
	auditReq.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_4_10_GeneratesAuditForLimitDeactivation(t *testing.T) {
	...
	// Query audit events
	auditReq, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?resourceId="+limitID+"&eventType=LIMIT_DEACTIVATED", nil)
	require.NoError(t, err)
	auditReq.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_4_11_GeneratesAuditForLimitDelete(t *testing.T) {
	...
	// Query audit events
	auditReq, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?resourceId="+limitID+"&eventType=LIMIT_DELETED", nil)
	require.NoError(t, err)
	auditReq.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_9_2_InvalidDateFormatReturns400(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?startDate=invalid-date", nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	assert.Equal(t, "Validation Error", errResp.Title, "Error response should have validation error title")
	assert.Contains(t, errResp.Message, "startDate must be in RFC3339 format", "Error message should indicate invalid date format")
}
...
func TestAuditEvents_11_9_5_InvalidSortByFieldReturns400(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/audit-events?sortBy=invalidField", nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	assert.Equal(t, http.StatusBadRequest, resp.StatusCode, "should return 400 Bad Request for invalid sortBy field")
	...
	assert.Equal(t, "Validation Error", errResp.Title, "Error response should have validation error title")
	assert.Contains(t, errResp.Message, "sortBy must be one of", "Error message should indicate invalid sortBy field")
}
...
func TestAuditEvents_11_9_6_EndDateBeforeStartDateReturns400(t *testing.T) {
	...
	assert.Equal(t, http.StatusBadRequest, resp.StatusCode, "should return 400 Bad Request when endDate is before startDate")
	...
	assert.Equal(t, "Validation Error", errResp.Title, "Error response should have validation error title")
	assert.Equal(t, "endDate must be after startDate", errResp.Message, "Error message should indicate invalid date range")
}
...
func TestAuditEvents_11_10_1_CompleteRuleLifecycleIsAudited(t *testing.T) {
	...
	// 1. Create rule
	ruleName := "lifecycle-test-" + uuid.New().String()[:8]
	ruleReq := testutil.RuleRequest{
		Name:       ruleName,
		Expression: "true",
		Action:     "ALLOW",
...
func TestAuditEvents_11_10_2_ValidationTriggersAuditEvent(t *testing.T) {
	...
	auditURL := fmt.Sprintf("%s/v1/audit-events?eventType=TRANSACTION_VALIDATED&resourceType=transaction",
		baseURL)
...
func TestAuditEvents_11_11_2_ActorInformationCaptured(t *testing.T) {
	...
	ruleName := "actor-test-" + uuid.New().String()[:8]
	ruleReq := testutil.RuleRequest{
		Name:       ruleName,
		Expression: "true",
		Action:     "ALLOW",
	...
	assert.Contains(t, []string{"user", "system"}, actor["actorType"], "actorType must be valid")
	assert.NotEmpty(t, actor["id"], "actor.id must be present")
	assert.NotEmpty(t, actor["name"], "actor.name must be present")
...
func TestAuditEvents_11_5_1_HashChainIntactAfterMultipleOperations(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		ruleName := fmt.Sprintf("chain-test-%d-%s", i, uuid.New().String()[:8])
		ruleReq := testutil.RuleRequest{
			Name:       ruleName,
			Expression: "true",
			Action:     "ALLOW",
...
func TestAuditEvents_11_6_1_FiltersBySegmentId(t *testing.T) {
	...
	// Filter by segmentId
	url := fmt.Sprintf("%s/v1/audit-events?segmentId=%s&eventType=TRANSACTION_VALIDATED", baseURL, segmentID)
	req, _ := http.NewRequest(http.MethodGet, url, nil)
	req.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_6_2_FiltersByPortfolioId(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/audit-events?portfolioId=%s&eventType=TRANSACTION_VALIDATED", baseURL, portfolioID)
	req, _ := http.NewRequest(http.MethodGet, url, nil)
	req.Header.Set("X-API-Key", apiKey)
...
func TestAuditEvents_11_6_3_CombinesMultipleJSONBFilters(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/audit-events?accountId=%s&transactionType=PIX&segmentId=%s&eventType=TRANSACTION_VALIDATED",
