...
func TestListRules_2_4_1_FilterByAccountId(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/rules?accountId=%s", baseURL, accountID)
	...
	assert.Contains(t, ruleIDs, ruleWithScope, "Should contain rule with matching accountId scope")
	assert.Contains(t, ruleIDs, ruleGlobal, "Should contain global rule (no scopes)")
	assert.NotContains(t, ruleIDs, ruleOtherScope, "Should NOT contain rule with different accountId scope")
}
...
func TestListRules_2_4_2_FilterByTransactionType(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/rules?transactionType=%s", baseURL, txType)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	ruleIDs := extractRuleIDsFromList(t, rules)
	assert.Contains(t, ruleIDs, ruleCard, "Should contain rule with CARD transactionType")
	assert.Contains(t, ruleIDs, ruleGlobal, "Should contain global rule")
	assert.NotContains(t, ruleIDs, rulePix, "Should NOT contain rule with PIX transactionType")
}
...
func TestListRules_2_4_3_FilterByMultipleScopeFields(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/rules?accountId=%s&transactionType=%s", baseURL, accountID, txType)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	assert.Contains(t, ruleIDs, ruleAccountOnly, "Should contain rule with wildcard transactionType")
	assert.Contains(t, ruleIDs, ruleTxTypeOnly, "Should contain rule with wildcard accountId")
}
...
func TestListRules_2_4_4_ScopeFiltersWithExistingFilters(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/rules?accountId=%s&action=DENY", baseURL, accountID)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
...
func TestListRules_2_4_5_ScopeFilterWithPagination(t *testing.T) {
	...
	// First page: limit=2
	url := fmt.Sprintf("%s/v1/rules?segmentId=%s&limit=2", baseURL, segmentID)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
...
func TestListRules_2_4_8_FilterBySegmentId(t *testing.T) {
	...
	url := fmt.Sprintf("%s/v1/rules?segmentId=%s", baseURL, segmentID)
	req, err := http.NewRequest(http.MethodGet, url, nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	ruleIDs := extractRuleIDsFromList(t, rules)
	assert.Contains(t, ruleIDs, ruleMatch, "Should contain rule with matching segmentId")
	assert.NotContains(t, ruleIDs, ruleNoMatch, "Should NOT contain rule with different segmentId")
}
