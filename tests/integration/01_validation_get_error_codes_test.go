...
// Returns TRC-0043 for invalid sortBy field and TRC-0042 for invalid sortOrder value.
func TestListTransactionValidations_InvalidSortParams_ReturnsError(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantCode:  "TRC-0043",
			wantTitle: "Invalid Transaction Validation Filters",
			wantMsg:   "sortBy must be one of [createdAt processingTimeMs]",
		},
		{
			...
			wantCode:  "TRC-0042",
			wantTitle: "Invalid Transaction Validation Filters",
			wantMsg:   "sortOrder must be ASC or DESC",
		},
		{
...
// TestListTransactionValidations_SortParamsWithCursor_ReturnsError verifies that sortBy/sortOrder
...
func TestListTransactionValidations_SortParamsWithCursor_ReturnsError(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			query:    "cursor=" + dummyCursor + "&sortBy=createdAt",
			wantCode: "TRC-0045",
			wantMsg:  "sortBy and sortOrder cannot be used with cursor; cursor already contains sort configuration",
		},
		{
			name:     "sortOrder-with-cursor",
			query:    "cursor=" + dummyCursor + "&sortOrder=ASC",
			wantCode: "TRC-0045",
			wantMsg:  "sortBy and sortOrder cannot be used with cursor; cursor already contains sort configuration",
		},
		{
			name:     "both-sort-params-with-cursor",
			query:    "cursor=" + dummyCursor + "&sortBy=createdAt&sortOrder=DESC",
			wantCode: "TRC-0045",
			wantMsg:  "sortBy and sortOrder cannot be used with cursor; cursor already contains sort configuration",
		},
	}
...
func TestListTransactionValidations_InvalidTransactionType_ReturnsError(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantCode: "TRC-0250",
			wantMsg:  "transactionType must be one of [CARD, WIRE, PIX, CRYPTO]",
		},
		{
			name:     "transactionType-lowercase",
			query:    "transactionType=card",
			wantCode: "TRC-0250",
			wantMsg:  "transactionType must be one of [CARD, WIRE, PIX, CRYPTO]",
		},
		{
			name:     "transactionType-cash",
			query:    "transactionType=CASH",
			wantCode: "TRC-0250",
			wantMsg:  "transactionType must be one of [CARD, WIRE, PIX, CRYPTO]",
		},
		{
			name:     "transactionType-numeric",
			query:    "transactionType=123",
			wantCode: "TRC-0250",
			wantMsg:  "transactionType must be one of [CARD, WIRE, PIX, CRYPTO]",
		},
	}
...
func TestListTransactionValidations_InvalidUUIDFilters_ReturnsError(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantCode: "TRC-0250",
			wantMsg:  "accountId must be a valid UUID",
		},
		{
			name:     "invalid-matchedRuleId-uuid",
			query:    "matchedRuleId=invalid-uuid",
			wantCode: "TRC-0250",
			wantMsg:  "matchedRuleId must be a valid UUID",
		},
		{
			name:     "invalid-exceededLimitId-uuid",
			query:    "exceededLimitId=12345",
			wantCode: "TRC-0250",
			wantMsg:  "exceededLimitId must be a valid UUID",
		},
		{
			name:     "invalid-segmentId-uuid",
			query:    "segmentId=abc",
			wantCode: "TRC-0250",
			wantMsg:  "segmentId must be a valid UUID",
		},
		{
			name:     "invalid-portfolioId-uuid",
			query:    "portfolioId=not-valid",
			wantCode: "TRC-0250",
			wantMsg:  "portfolioId must be a valid UUID",
		},
		...
		// so it won't fail validation. We test with a truly invalid format instead.
		{
			...
			query:    "accountId=550e8400-e29b-41d4-a716-44665544000g",
			wantCode: "TRC-0250",
			wantMsg:  "accountId must be a valid UUID",
		},
		{
			name:     "partial-uuid",
			query:    "accountId=550e8400-e29b",
			wantCode: "TRC-0250",
			wantMsg:  "accountId must be a valid UUID",
		},
	}
...
func TestListTransactionValidations_InvalidDateFilters_ReturnsError(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantCode: "TRC-0020",
			wantMsg:  `startDate must be in RFC3339 format with timezone (e.g., 2026-01-28T10:30:00Z). Invalid value: "2024-01-01"`,
		},
		{
			name:     "invalid-endDate-format",
			query:    "endDate=invalid-date",
			wantCode: "TRC-0020",
			wantMsg:  `endDate must be in RFC3339 format with timezone (e.g., 2026-01-28T10:30:00Z). Invalid value: "invalid-date"`,
		},
		{
			name:     "startDate-after-endDate",
			query:    "startDate=2024-12-31T23:59:59Z&endDate=2024-01-01T00:00:00Z",
			wantCode: "TRC-0023",
			wantMsg:  "startDate must be before or equal to endDate",
		},
		{
			name:     "date-without-timezone",
			query:    "startDate=2024-01-01T12:00:00",
			wantCode: "TRC-0020",
			wantMsg:  `startDate must be in RFC3339 format with timezone (e.g., 2026-01-28T10:30:00Z). Invalid value: "2024-01-01T12:00:00"`,
		},
		{
			name:     "date-numeric",
			query:    "startDate=1704067200",
			wantCode: "TRC-0020",
			wantMsg:  `startDate must be in RFC3339 format with timezone (e.g., 2026-01-28T10:30:00Z). Invalid value: "1704067200"`,
		},
	}
...
func TestListTransactionValidations_MultipleInvalidParams_ReturnsFirstError(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/validations?limit=-1&decision=INVALID&sortBy=badField", nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
...
// TestListTransactionValidations_CursorWithSortBy_Rejected verifies TRC-0045: cursor cannot be used with sortBy.
func TestListTransactionValidations_CursorWithSortBy_Rejected(t *testing.T) {
	baseURL := testutil.GetBaseURL()
	apiKey := testutil.GetAPIKey()

	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/validations?cursor=eyJpZCI6InRlc3QiLCJwbiI6dHJ1ZX0=&sortBy=createdAt", nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	assert.Contains(t, errResp.Message, "sortBy and sortOrder cannot be used with cursor")
}

// TestListTransactionValidations_CursorWithSortOrder_Rejected verifies TRC-0045: cursor cannot be used with sortOrder.
func TestListTransactionValidations_CursorWithSortOrder_Rejected(t *testing.T) {
	baseURL := testutil.GetBaseURL()
	apiKey := testutil.GetAPIKey()

	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/validations?cursor=eyJpZCI6InRlc3QiLCJwbiI6dHJ1ZX0=&sortOrder=ASC", nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
...
// TestListTransactionValidations_CursorWithBothSortParams_Rejected verifies TRC-0045: cursor cannot be used with both sortBy and sortOrder.
func TestListTransactionValidations_CursorWithBothSortParams_Rejected(t *testing.T) {
	baseURL := testutil.GetBaseURL()
	apiKey := testutil.GetAPIKey()

	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/validations?cursor=eyJpZCI6InRlc3QiLCJwbiI6dHJ1ZX0=&sortBy=createdAt&sortOrder=ASC", nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
...
// TestListTransactionValidations_InvalidSortOrder_TRC0042 verifies TRC-0042: sortOrder must be ASC or DESC.
func TestListTransactionValidations_InvalidSortOrder_TRC0042(t *testing.T) {
	baseURL := testutil.GetBaseURL()
	apiKey := testutil.GetAPIKey()

	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/validations?sortOrder=INVALID", nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	assert.Contains(t, errResp.Message, "sortOrder must be ASC or DESC")
}

// TestListTransactionValidations_InvalidSortBy_TRC0043 verifies TRC-0043: sortBy must be in allowed list.
func TestListTransactionValidations_InvalidSortBy_TRC0043(t *testing.T) {
	baseURL := testutil.GetBaseURL()
	apiKey := testutil.GetAPIKey()

	req, err := http.NewRequest(http.MethodGet, baseURL+"/v1/validations?sortBy=invalidField", nil)
	require.NoError(t, err)
	req.Header.Set("X-API-Key", apiKey)
	...
	assert.Contains(t, errResp.Message, "sortBy must be one of [createdAt processingTimeMs]")
}
...
func TestListTransactionValidations_Timeout_WithFilters_ReturnsTRC0252(t *testing.T) {
	...
	queryParams := "decision=ALLOW&transactionType=CARD&sortBy=processingTimeMs&sortOrder=DESC"
